...
import { experiences } from "../../helpers/serverExperience"
import { currentUser, useStore } from "../../store";

export const ExpCard = ({ el}:{el: experiences}) => {
  const userStore = useStore()
  const currStore = currentUser()
  const navigate = useNavigate();
  const navegar = () => {
      navigate("/experience/form", { state: { element: el } });
  }
  ...
  return (
    <article className="relative flex flex-col gap-3 w-[96%]">
        <h2 className="mt-3 font-semibold text-[#F4E559] text-[32px]">{el.role}</h2>
        <p className="text-2xl text-white">{el.company} {el.contract && `· ${el.contract}`}</p>
